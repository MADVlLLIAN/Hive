...
const { readWavMusicBeeTags } = require('../app/main/wav-id3');
...
test('WAV scanner treats Love as true when the same field contains both U and L values', async () => {
  const multiValueLove = (encoding = 3) => {
    const separator = encoding === 1 || encoding === 2 ? Buffer.from([0, 0]) : Buffer.from([0]);
    return frame('TXXX', Buffer.concat([Buffer.from([encoding]), encode('LOVE RATING', encoding), separator, encode('U', encoding), separator, encode('L', encoding)]));
  };
  for (const encoding of [0, 3, 1, 2]) {
    await withFixture(wav([chunk('id3 ', id3([multiValueLove(encoding)]))]), async file => {
      assert.equal((await readWavMusicBeeTags(file)).loved, true, `encoding ${encoding}`);
    });
  }
});

test('WAV scanner treats Love as true when separate U and L fields exist', async () => {
  await withFixture(wav([chunk('id3 ', id3([loveFrame('U'), loveFrame('L')]))]), async file => {
    assert.equal((await readWavMusicBeeTags(file)).loved, true);
  });
});
