...
def _decode_freeform_component(value):
    # MP4 freeform mean/name fields are UTF-8 text in normal iTunes-style
    # metadata. Fall back to the historical latin-1 behavior for malformed or
    # legacy files so reading an unusual existing atom remains non-fatal.
    try:
...
def _encode_freeform_component(value):
    # Unlike four-byte MP4 atom identifiers, freeform names are not limited to
    # latin-1. Keep the four-byte `----` atom itself ASCII, but encode its mean
    # and name components as UTF-8 so Unicode custom fields can round-trip.
...
def _key2name(key):
    if key.startswith("----:"):
        try:
            _dummy, mean, name = key.split(":", 2)
        except ValueError:
            return key.encode("latin-1")
        return b"----:" + _encode_freeform_component(mean) + b":" + _encode_freeform_component(name)
    return key.encode("latin-1")
...
class MP4Tags(DictProxy, Tags):
    ...
    * 'gnre' -- ID3v1 genre. Not supported, use '\\xa9gen' instead.
    ...
    def __parse_freeform(self, atom, data):
        ...
        key = "----:%s:%s" % (_decode_freeform_component(mean), _decode_freeform_component(name))
