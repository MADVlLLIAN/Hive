  ...
  for (let pos = 0; pos + 10 <= payload.length;) {
    ...
    if (id === 'POPM') {
      ...
      if ([0, 1, 2, 3].includes(encoding)) {
        ...
        if (isBeehiveLoveFieldName(decodeId3TextBytes(description, encoding).replace(/^\uFEFF/, '').trim()) && splitId3TextValues(value, encoding).some(isFavoriteLoveValue)) result.loved = true;
      }
    }
    pos += 10 + size;
  }
...
module.exports = { readWavMusicBeeTags, readWavMusicBeeLove, readWavMusicBeePopmRaw, parseId3LoveAndRating, decodeId3TextBytes, splitId3TextValues, isBeehiveLoveFieldName, isFavoriteLoveValue, musicBeePopmStars };
