...
function createSessionBusWithoutUsocket() {
  const configured = String(process.env.DBUS_SESSION_BUS_ADDRESS || '');
  // dbus-next@0.10.2 selects its native usocket module for unix:path and
  // unix:abstract addresses. The old usocket ABI is unsafe in Electron 44.
  // The normal Linux session bus is a filesystem socket, which Node's net
  // module supports directly when expressed as unix:socket=...; MPRIS does
  // not send file descriptors, so Unix-FD negotiation can remain disabled.
  const match = configured.match(/(?:^|;)unix:path=([^;,]+)/);
  if (match) {
    const socketPath = decodeURIComponent(match[1]);
    return dbus.sessionBus({ busAddress: `unix:socket=${socketPath}`, negotiateUnixFd: false });
  }
  const runtimeDir = process.env.XDG_RUNTIME_DIR;
  const fallbackPath = runtimeDir && path.join(runtimeDir, 'bus');
  if (fallbackPath && fs.existsSync(fallbackPath)) {
    return dbus.sessionBus({ busAddress: `unix:socket=${fallbackPath}`, negotiateUnixFd: false });
  }
  // Preserve dbus-next's normal discovery for nonstandard buses. If that bus
  // uses an abstract socket, its existing failure is contained by start().
  return dbus.sessionBus({ negotiateUnixFd: false });
}
...
class PlayerInterface extends (dbus?.interface?.Interface || class {}) {
  ...
  get CanGoNext() { return !!this.owner.state.canGoNext; }
  get CanGoPrevious() { return !!this.owner.state.canGoPrevious; }
...
class BeehiveMPRIS {
  constructor({ userData, sendCommand, artworkProxy }) {
    this.userData = userData; this.sendCommand = sendCommand; this.artworkProxy = artworkProxy || null; this.bus = null; this.root = null; this.player = null; this.running = false;
    this.state = { track: null, position: 0, duration: 0, paused: true, volume: 1, shuffle: false, repeat: 0, canGoNext: false, canGoPrevious: false, artworkPath: '' };
    this.trackKey = ''; this.coverPath = ''; this.lastMetadataKey = ''; this.lastPosition = 0; this.lastPlaybackStatus = ''; this.lastArtworkUrl = '';
    this.reconnectTimer = null; this.reconnectDelayMs = 1000; this.connecting = false; this.stopping = false;
  }
  command(command) { try { this.sendCommand(command); } catch {} }
  _scheduleReconnect(reason) {
    if (this.stopping || process.platform !== 'linux' || !dbus || this.reconnectTimer || this.running || this.connecting) return;
    console.warn('[MPRIS] Session bus disconnected; scheduling automatic re-registration:', reason || 'unknown');
  ...
  _handleBusError(error) {
    const message = error?.message || String(error || 'unknown error');
    console.warn('[MPRIS] D-Bus bus error:', message);
    this.running = false;
    this.root = null;
    this.player = null;
  ...
  async start() {
    if (!dbus || this.running || this.connecting || process.platform !== 'linux') return false;
    this.stopping = false;
    this.connecting = true;
    let bus = null;
    try {
      bus = createSessionBusWithoutUsocket();
      this.bus = bus;
      bus.on?.('error', (error) => { if (this.bus === bus) this._handleBusError(error); });
      bus.on?.('close', () => { if (this.bus === bus) this._handleBusClose(); });
      const reply = await bus.requestName(SERVICE, dbus.NameFlag?.DO_NOT_QUEUE ?? 4);
      const primaryOwner = dbus.RequestNameReply?.PRIMARY_OWNER ?? 1;
      const alreadyOwner = dbus.RequestNameReply?.ALREADY_OWNER ?? 4;
      if (reply !== primaryOwner && reply !== alreadyOwner) {
        throw new Error(`D-Bus name ${SERVICE} was not acquired (reply=${String(reply)})`);
      }
      this.root = new RootInterface(); this.player = new PlayerInterface(this);
      bus.export(OBJECT_PATH, this.root); bus.export(OBJECT_PATH, this.player); this.running = true;
      console.info('[MPRIS] Registered', SERVICE);
      await this.update(this.state);
      return true;
    } catch (e) {
      console.warn('[MPRIS] Registration failed:', e?.message || e);
      try { bus?.disconnect(); } catch {}
      if (this.bus === bus) { this.bus = null; this.root = null; this.player = null; this.running = false; }
      this._scheduleReconnect(e?.message || e);
      return false;
    } finally {
  ...
  stop() {
    ...
    this.bus=null; this.root=null; this.player=null; this.running=false; this.connecting=false;
  }
  metadata() {
    ...
    if(t.path){
      const raw=String(t.path);
      const uri=/^(?:https?:|spotify:)/i.test(raw) ? raw : pathToFileURL(raw).href;
      m['xesam:url']=new v('s',uri);
    }
    ...
    const localArt = this.coverPath && fs.existsSync(this.coverPath)
      ...
      : '';
    if (localArt) {
      // Keep the MPRIS contract compatible with the known-good pre-proxy
      // implementation. Music Presence is responsible for turning local
      // file:// artwork into a Discord-safe public URL; Hive's optional
      // localhost artwork proxy must not replace the player-provided URI.
  ...
  async writeCover() {
    ...
    if (!t?.path || /^(?:https?:|spotify:)/i.test(String(t.path)) || !requested) { this.coverPath = ''; return; }
    try {
  ...
  async update(p={}) {
    ...
    // Metadata identity must track the same artwork URI that MPRIS exposes.
    // Keep local covers as file:// URIs; Music Presence owns externalizing
    // local artwork and this avoids the localhost-proxy regression.
    const artworkUrl = (this.coverPath && fs.existsSync(this.coverPath))
      ? pathToFileURL(this.coverPath).href
      : String(this.state.artworkUrl || '');
    ...
    if (Math.abs(Number(this.state.position||0)-Number(this.lastPosition||0)) >= 1 || Object.keys(changed).length) {
      ...
      try { dbus.interface.Interface.emitPropertiesChanged(this.player, changed, []); } catch(e) {
        console.warn('[MPRIS] PropertiesChanged failed:', e?.message || e);
        this.running = false;
        this.root = null;
        this.player = null;
        this._scheduleReconnect(e?.message || e);
