...
async function runSecurityAudit(root) {
  const main = await readText(path.join(root, 'main.js'));
  const preload = await readText(path.join(root, 'preload.js'));
  const html = await readText(path.join(root, 'src', 'index.html'));
  ...
  const checks = [
    ...
    ['csp', /Content-Security-Policy/i.test(html) && /script-src\s+['"]self['"]/i.test(html), 'Content Security Policy', 'The renderer declares a restrictive self-only script policy.'],
...
async function runEnvironmentAudit(root, paths = {}) {
  ...
  const gst = path.join(root, 'gstreamer-player.c');
  try {
  ...
  const required = ['main.js', 'preload.js', 'src/renderer.js', 'resources/tag_helper.py', 'database-worker.py', 'task-manager.js'];
