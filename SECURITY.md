...
## Build 38 security hardening

Build 38 removes installer-side remote shell execution for Spicetify and SpotX, uses per-user ACL guidance for Spotify filesystem access, authenticates the local Spotify bridge with a per-user token, and writes Hive JSON state with restrictive file permissions. The trusted-local plugin API remains intentionally privileged and should only load extensions the user trusts.
